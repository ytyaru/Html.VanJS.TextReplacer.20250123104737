;(function(){
class LineParser { // 原稿のうちフェンス以外（単一行で表現された部分）をブロックにして返却する
...
//    set preprocess(v) {if ('function'===typeof v){this._preprocess = v}}
    parse(script, fbs) { // script:原稿（簡易構文）, fbs:フェンスブロック配列
        ...
        return this.#getWalls(script, fbs);
...
//        return this.#getBlocks(walls)
    ...
    #getWalls(script, fbs) { // script:原稿（簡易構文）, fbs:フェンスブロック配列
        ...
        return walls.filter(w=>w.script.text.trim())
    }
    #makeWall(script, fbs, start, end) {
        //const wall = {start:start, end:end, script:script.slice(start, end)}
        const wall = {script:{start:start, end:end, text:script.slice(start, end)}, parse:{data:null}}
        console.log(wall)
        //wall.html = this.#parseWall(wall.script, fbs)
        //wall.data = this.#parseWall(wall.script, fbs)
//        if (!wall.parse) {wall.parse={}}
        //wall.parse.data = this.#parseWall(wall.script, fbs)
        wall.parse.data = this.#parseWall(wall.script.text, fbs)
        return wall
    }
    #parseWall(script, fbs) {return script.split('\n').filter(v=>v).map(line=>this.#parseLine(line))}
    /*
...
//    parse(script, fbs) { // script:原稿（簡易構文）, fbs:フェンスブロック配列
        ...
        throw new TypeError(`不正な書式です。:${script}`)
    }
    */
    //#parseLine(script) {
    #parseLine(text) {
        ...
        throw new TypeError(`不正な書式です。:${text}`)
    }
    /*
    #parseWall(script, fbs) {
...
//            const textValues = this._S.split(match.groups.valueText);
            const textValues = Splitter.split(match.groups.valueText);
            ...
            const type = TypeParser.read(typeT, defVal, textValues);
...
//            const rowsValues = this._S.split(match.groups.rowsText);
            const rowsValues = Splitter.split(match.groups.rowsText);
...
//            console.log(name, type)
    ...
    */
    #getType(typeTxt, defTxt, rowsValues, colLen, colIdx) {
        ...
        const type = TypeParser.getType(typeTxt, defTxt)
        if (type){return type}
        else {// typeが無いなら値で型を推測する
            for (let i=0; i<rowsValues.length/colLen; i++){
                const v = rowsValues[(colLen*i)+colIdx]
...
window.LineParser = LineParser;
window.textbase = {}
window.textbase.Ktv = Ktv;
window.textbase.Splitter = Splitter;
window.textbase.LiteralType = LiteralType;
window.textbase.TypeParser = TypeParser;
window.textbase.DataType = DataType;
window.textbase.StringDataType = StringDataType;
window.textbase.BooleanDataType = BooleanDataType;
window.textbase.FloatDataType = FloatDataType;
window.textbase.IntDataType = IntDataType;
window.textbase.BigIntDataType = BigIntDataType;
//window.textbase.DataType = DataType;
//window.textbase.DataType = DataType;
//window.textbase.DataType = DataType;
})();
